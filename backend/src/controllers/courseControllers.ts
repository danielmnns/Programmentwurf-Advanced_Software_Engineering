...
// Benutzer austragen
export const unenrollUser = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const course = await CourseModel.findById(req.params.id);
    if (!course) {
      return res.status(404).json({ message: 'Course not found' });
...
export const addActivity = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
// Aktivität entfernen
export const removeActivity = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const course = await CourseModel.findById(req.params.id);
    if (!course) {
      return res.status(404).json({ message: 'Course not found' });
...
export const getEnrolledUsers = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
export const getCourseInfo = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const course = await CourseModel.findById(req.params.id);
    if (!course) {
      return res.status(404).json({ message: 'Course not found' });
